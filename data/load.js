...
var hashes = JSON.parse(fs.readFileSync('data/hashes.json', 'utf8'));
var w3c = require("w3capi");
w3c.apiKey = JSON.parse(fs.readFileSync('data/key.json', 'utf8'));
...
function finalize() {
  if (responded === requested) {
    fs.writeFileSync('data/persons.json', JSON.stringify(Object.keys(persons).map( function(p) { return persons[p];})));
  }
}
    ...
    w3c.user(h).groups().fetch( { embed: true },
      function(error, data) {
        ...
        if (!error) {
          ...
          persons[h].groups = groups.map(function(g) { return g.name;});
        }
        finalize();
      });
});
